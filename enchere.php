...
// ⏰ Vérifier si 20 secondes sont passées depuis la dernière enchère
$bid_time = strtotime($enchere['BidTime']);
$now = time();

if ($now - $bid_time >= 20) {
    $last_user_id = $enchere['UserID'];
    $final_price = $enchere['BidAmount'];

    // Créer ou récupérer le panier
    $sql_panier = "SELECT PanierID FROM Panier WHERE UserID = ? AND Status = 'En cours'";
    $stmt_panier = $conn->prepare($sql_panier);
    $stmt_panier->bind_param("i", $last_user_id);
    $stmt_panier->execute();
    $res_panier = $stmt_panier->get_result();

    if ($res_panier->num_rows > 0) {
        $panier = $res_panier->fetch_assoc();
        $panier_id = $panier['PanierID'];
    } else {
        $sql_new_panier = "INSERT INTO Panier (UserID, Status) VALUES (?, 'En cours')";
        $stmt_new = $conn->prepare($sql_new_panier);
        $stmt_new->bind_param("i", $last_user_id);
        $stmt_new->execute();
        $panier_id = $conn->insert_id;
    }

    // Ajouter l'article au panier
    $sql_add = "INSERT INTO PanierArticles (PanierID, ArticleID, Quantity) VALUES (?, ?, 1)
                ON DUPLICATE KEY UPDATE Quantity = Quantity + 1";
    $stmt_add = $conn->prepare($sql_add);
    $stmt_add->bind_param("ii", $panier_id, $article_id);
    $stmt_add->execute();

    // Créer une notification
    $sql_notify = "INSERT INTO Notifications (UserID, TypeVente, TypeAchat, Rarete, Quantity, CreatedAt, Message)
                   VALUES (?, 'enchere', '', '', 1, NOW(), 'Vous avez remporté l\'enchère. Cliquez pour payer.')";
    $stmt_notify = $conn->prepare($sql_notify);
    $stmt_notify->bind_param("i", $last_user_id);
    $stmt_notify->execute();
    ...
    <div id="timer">Temps restant : <span id="time">20</span> secondes</div>
...
const countdown = setInterval(() => {
    ...
    if (timeLeft <= 0) {
        ...
        location.reload();  // recharge pour déclencher la logique PHP ci-dessus
    }
}, 1000);
</script>
